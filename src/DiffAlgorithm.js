 ...
 */
 ...
 function patienceDiff(aLines, bLines, diffPlusFlag) {
   ...
   *
   */
  const a = 'map() 메서드는 배열 내의 모든 요소 각각에 대하여 주어진 함수를 호출한 결과를 모아 새로운 배열을 반환합니다.'
  
  const b = 'map() (뭔가를 빼고 추가함) 배열 내의 모든 요소 각각에 대하여 (뭔가를 추가함) 주어진 함수를 호출한 결과를 모아 새로운 배열을 반환합니다.'
  
  
  
  let difference1 = patienceDiff( a.split(""), b.split(""));

  console.log(difference1)
  
  const result = difference1.lines.filter(el => el.aIndex === -1)
    
  // 1. 클라이언트에서 서버쪽에 데이터를 넘겨줄 때, 변수 b를 넘겨주게 된다.
  // 2. 서버에서 기존 데이터베이스에 저장되어 있는 변수 a와 클라이언트에서 넘겨준 변수 b를 차이 알고리즘(위에 있는 코드)을 이용하여 비교한다
  //     -> 결과값에서 aIndex가 -1인 것만 필터한다. 여기서 aIndex가 -1이라는 의미는 변수 a에 없는 string이라는 뜻
  // 3. 클라이언트에서 넘겨준 변수 b를 데이터베이스에 업데이트 해준다.
  // 4. 2번에서 필터한 값들을 가진 배열을 응답에 포함하여 클라이언트에 전송한다.
  // 5. 클라이언트에서는 응답받은 데이터를 이용하여 
